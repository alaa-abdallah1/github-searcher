...
export const fetchItems = createAsyncThunk(
  ...
  async (args: SearchArgs, {rejectWithValue, getState}) => {
    ...
    try {
      const {data} = await client.get(`/${entity}`, {params})
      return data
    } catch (err: unknown) {
    ...
    builder
      ...
      .addCase(
        ...
        (state, action: PayloadAction<FetchItemsResponse>) => {
          ...
          state.items = state.isFetchingMore
            ? [...state.items, ...payload.items]
            : payload.items

          state.hasMore =
            state.items.length !== payload.total_count && !!payload.items.length
...
export const {setQuery, setEntity, resetItems, setIsFetchingMore} =
  githubSlice.actions
